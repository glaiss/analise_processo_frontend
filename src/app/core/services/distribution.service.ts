...
export class DistributionService {
  ...
  getMeusProcessos(page: number = 0, size: number = 20, filter?: ProcessoFilterParams): Observable<Page<AtribuicaoProcessoResumoDTO>> {
  ...
  getProcessosEquipe(page: number = 0, size: number = 20, filter?: ProcessoFilterParams): Observable<Page<AtribuicaoProcessoResumoDTO>> {
    let params = new HttpParams().set('page', page.toString()).set('size', size.toString());
    params = this.appendFilterParams(params, filter);
  ...
  private appendFilterParams(params: HttpParams, filter?: ProcessoFilterParams): HttpParams {
    ...
    if (filter.niveis) { filter.niveis.forEach(n => { params = params.append('niveis', n); }); }
    if (filter.status) { filter.status.forEach(s => { params = params.append('status', s); }); }
    if (filter.tribunal) { params = params.set('tribunal', filter.tribunal); }
    if (filter.assunto) { params = params.set('assunto', filter.assunto); }
    if (filter.situacao) { filter.situacao.forEach(s => { params = params.append('situacao', s); }); }
    return params;
  }
